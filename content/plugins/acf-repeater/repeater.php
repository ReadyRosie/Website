...
class acf_field_repeater extends acf_field
{
    ...
    */
    ...
    function get_dir()
    {
        ...
        $dir = str_replace($wp_content_dir, WP_PLUGIN_URL, $dir);
	...
	function load_field( $field )
	{
		...
		if( isset($field['sub_fields']) && is_array($field['sub_fields']) )
		{
			foreach( $field['sub_fields'] as $k => $sub_field )
			{
				...
				$sub_field = apply_filters('acf/load_field-' . $sub_field['type'], $sub_field); // new filter
				
				
				// extra filters for parent-child
				...
				foreach( $keys as $key )
				{
					...
					$sub_field = apply_filters('acf/load_field-' . $action, $sub_field); // new filter
					
				}
				
				
				// general load_field filter
	...
	function update_field( $field, $post_id )
	{
		...
		if( $field['sub_fields'] )
		{
			...
			foreach( $field['sub_fields'] as $key => $f )
			{
				...
				// save
				$f = apply_filters('acf/update_field-' . $f['type'], $f, $post_id ); // new filter
