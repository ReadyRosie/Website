...
function getscheduledposts($type) {
    	global $wpdb;
    	$thetitletype = "order_email_post." . $type;
    	$sql="
    	
SELECT order_email_post.numOrder, wp_posts.ID, wp_posts.post_title, wp_posts.post_status, wp_posts.post_type
FROM wp_posts
INNER JOIN order_email_post ON $thetitletype = wp_posts.post_title
WHERE wp_posts.post_status <> 'trash'
AND wp_posts.post_status <> 'inherit'
AND wp_posts.post_type = 'video'
ORDER BY order_email_post.numOrder asc
...
function sspe_add_menu_page_callback() {
    ...
    if( isset($_POST[ $sspe_hidden ]) && $_POST[ $sspe_hidden ] == 'Y' ) {
		...
		<?php 
		_e('settings saved. post-id: ', 'sspe'); 
		_e($_POST[ $sspe_post1 ], 'sspe'); 
		_e(' - on: ', 'sspe'); 
		
		$theunixtime = strtotime( $_POST[ $sspe_timestamp ] );
		$posteddate = strftime('%A %B %e',$theunixtime);
	...
	<div class="wrap">
		...
		<form name="sspe" method="post" action="">
			...
			<div style="float: left; width: 65%;">
				...
				<?php $scheduledposts_english = getscheduledposts('txtEnglishTitle'); ?>
				<?php $scheduledposts_spanish = getscheduledposts('txtSpanishTitle'); ?>
				<?php $scheduledposts_expert = getscheduledposts('txtExpertTitle'); ?>
						...
						<tr valign="top">
							...
							<td>
								<select name="<?php echo $sspe_post1; ?>" data-placeholder="Pick Today's Video..." style="width:350px;" class="chzn-select">
									...
									<?php foreach( $scheduledposts_english as $englishpost ): ?>
									<option value="<?php echo $englishpost->ID; ?>"><?php echo $englishpost->numOrder; ?> - <?php echo $englishpost->ID; ?> - <?php echo $englishpost->post_title; ?></option>
									<?php endforeach; ?>
						...
						<tr valign="top">
							...
							<td>
								<select name="<?php echo $sspe_post2; ?>" data-placeholder="Pick the expert video..." style="width:350px;" class="chzn-select">
									...
									<?php foreach( $scheduledposts_expert as $expertpost ): ?>
									<option value="<?php echo $expertpost->ID; ?>"><?php echo $expertpost->numOrder; ?> - <?php echo $expertpost->ID; ?> - <?php echo $expertpost->post_title; ?></option>
									<?php endforeach; ?>
						...
						<tr valign="top">
							...
							<td>
								<select name="<?php echo $sspe_post3; ?>" data-placeholder="Pick the Spanish video..." style="width:350px;" class="chzn-select">
									...
									<?php foreach( $scheduledposts_spanish as $spanishpost ): ?>
									<option value="<?php echo $spanishpost->ID; ?>"><?php echo $spanishpost->numOrder; ?> - <?php echo $spanishpost->ID; ?> - <?php echo $spanishpost->post_title; ?></option>
									<?php endforeach; ?>
			...
			<?php foreach ( $cron as $timestamp => $cronhooks ) { ?>
				<?php foreach ( (array) $cronhooks as $hook => $events ) { $i = ''; ?>
					<?php foreach ( (array) $events as $event ) { $i++; ?>
						...
						?>
						<tr>							
							<td><?php echo $tmpCount++; echo ": "; if ( count( $event[ 'args' ] ) ) { ?>
								<?php
								$api = new MCAPI( $sspe_options['sspe_mailchimp_api_key'] );
								...
								?>
							<?php } ?></td>
							<td scope="row"><?php echo date( 'D M d, Y g:ia T', $timestamp ); ?></td>
							<td>
								<?php echo $timestamp;
									if ( $event[ 'schedule' ] ) {
